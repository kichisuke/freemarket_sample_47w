Rails.application.routes.draw do
  get 'test', to: 'test#index'
  get 'test/mypage', to: 'test#mypage', as: 'mypage'
  get 'test/mypage/profile', to: 'test#profile'
  get 'test/purchase', to: 'test#purchase', as: 'purchase'
  get 'test/signup', to: 'test#signup'
  get 'test/login', to: 'test#login'
  get 'test/registration', to: 'test#registration'
  get 'test/sms_confirmation', to: 'test#sms_confirmation'
  get 'test/input_address', to: 'test#input_address'
  get 'test/payment', to: 'test#payment'
  get 'test/signup_end', to: 'test#signup_end'
  root 'test#index'
  get 'test/sell', to: 'test#sell'
  get 'detail', to: 'test#detail'
end
